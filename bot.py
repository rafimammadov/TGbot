"""
Telegram Voice Recording Bot — text + normalized_text sentences.
After sending a voice message, user must confirm or re-record.
"""
...
CHOOSE_AUTH, GET_PHONE, GET_USERNAME = range(3)
...
def _get_current_cycle(con, telegram_id):
    row = con.execute("SELECT MAX(cycle) FROM user_progress WHERE telegram_id=?", (telegram_id,)).fetchone()
    return row[0] or 1
...
def get_next_sentence(telegram_id, skip_set=None):
    ...
    with get_con() as con:
        ...
        if remaining == 0:
            cycle += 1
            con.execute("UPDATE user_progress SET cycle=?, recorded=0 WHERE telegram_id=?", (cycle, telegram_id))
        ...
        if effective_skip:
            ...
            """.format(",".join("?" * len(effective_skip))),
                [telegram_id, cycle] + list(effective_skip)).fetchone()[0]
            if non_skipped == 0:
                effective_skip = set()
        ...
        if effective_skip:
            ...
            row = con.execute(f"""
                ...
                ORDER BY up.sentence_id LIMIT 1
        ...
        total = con.execute("SELECT COUNT(*) FROM sentences WHERE is_active=1").fetchone()[0]
...
def upsert_user(telegram_id, username, phone, auth_method, first_name, last_name):
    with get_con() as con:
        con.execute("""
            INSERT INTO users (telegram_id,username,phone,auth_method,first_name,last_name,joined_at)
            VALUES (?,?,?,?,?,?,?)
            ON CONFLICT(telegram_id) DO UPDATE SET
                username=excluded.username, phone=excluded.phone,
                auth_method=excluded.auth_method,
                first_name=excluded.first_name, last_name=excluded.last_name
        """, (telegram_id,username,phone,auth_method,first_name,last_name,datetime.utcnow().isoformat()))
        return con.execute("SELECT id FROM users WHERE telegram_id=?", (telegram_id,)).fetchone()["id"]
...
def save_recording(user_db_id, telegram_id, sentence_id, text, normalized_text,
    ...
    with get_con() as con:
        ...
        """, (user_db_id,telegram_id,sentence_id,text,normalized_text,file_id,
              file_path,duration,file_size,mime_type,datetime.utcnow().isoformat()))
...
async def _show_sentence(message, telegram_id, ctx, is_skip=False):
    if count_sentences() == 0:
        await message.reply_text("⚠️ No sentences in DB yet. Ask admin to import.",
    ...
    # Clear any pending confirmation when showing a new sentence
    ctx.user_data.pop("pending_file_id",    None)
    ctx.user_data.pop("pending_file_path",  None)
    ctx.user_data.pop("pending_duration",   None)
    ctx.user_data.pop("pending_file_size",  None)
    ctx.user_data.pop("pending_mime_type",  None)
...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "👋 *Welcome to the Voice Recording Bot!*\n\nPlease identify yourself:",
        parse_mode="Markdown",
...
async def choose_auth(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if q.data == "auth_phone":
        await q.message.reply_text("Tap to share your number:",
            reply_markup=ReplyKeyboardMarkup(
                [[KeyboardButton("📱 Share my phone number", request_contact=True)]],
                one_time_keyboard=True, resize_keyboard=True))
        return GET_PHONE
    await q.message.reply_text("Type your username (no @ needed):", reply_markup=ReplyKeyboardRemove())
    return GET_USERNAME

async def get_phone(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    c = update.message.contact; u = update.effective_user
    if not c:
        await update.message.reply_text("Please use the button."); return GET_PHONE
    upsert_user(u.id, u.username, c.phone_number, "phone", u.first_name, u.last_name)
    await update.message.reply_text(f"✅ Registered! Phone: `{c.phone_number}`",
...
async def get_username(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    u = update.effective_user; username = update.message.text.strip().lstrip("@")
    if not username:
        await update.message.reply_text("Can't be empty. Try again:"); return GET_USERNAME
    upsert_user(u.id, username, None, "username", u.first_name, u.last_name)
    await update.message.reply_text(f"✅ Registered! Username: `@{username}`", parse_mode="Markdown")
    await update.message.reply_text("What would you like to do?", reply_markup=main_menu())
    return ConversationHandler.END
...
async def callback_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if q.data == "menu":
        ...
        if not file_id or not sentence_id:
            await q.message.reply_text("⚠️ Nothing to save. Please record a voice message first.",
                                       reply_markup=main_menu())
            return

        uid = get_user_db_id(u.id)
        save_recording(uid, u.id, sentence_id, text, normalized_text,
                       file_id, file_path, duration, file_size, mime_type or "audio/ogg")
        mark_recorded(u.id, sentence_id)

        # Write sidecar JSON
        if file_path:
            try:
                with open(Path(file_path).with_suffix(".json"), "w") as f:
                    json.dump({"telegram_id": u.id, "username": u.username,
                               "sentence_id": sentence_id, "text": text,
                               "normalized_text": normalized_text, "file_path": file_path,
                               "duration_sec": duration, "file_size_bytes": file_size,
        ...
        # ── Discard pending, show same sentence again ───────────────────────
        ...
        await q.message.reply_text(
            "ℹ️ You see the original text for reference and read the normalized version aloud.\n"
            "Every sentence is shown once before any repeats.",
            reply_markup=main_menu())

async def handle_voice(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    u = update.effective_user; v = update.message.voice
    uid = get_user_db_id(u.id)
    if not uid:
    ...
    if not sentence_id:
        info = get_next_sentence(u.id)
        if not info:
            await update.message.reply_text("⚠️ No sentences available."); return
        sentence_id     = info["sentence_id"]
        text            = info["text"]
        normalized_text = info["normalized_text"]
        ctx.user_data.update({
    ...
    # Download to temp folder (not permanent yet)
    tg_file  = await ctx.bot.get_file(v.file_id)
    filename = f"{u.id}_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}_{sentence_id}.ogg"
    dest     = TEMP_DIR / filename
    await tg_file.download_to_drive(dest)

    # Store pending info in context — not saved to DB yet
    ctx.user_data.update({
...
async def cmd_admin(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    with get_con() as con:
        ...
        secs  = con.execute("SELECT COALESCE(SUM(duration_sec),0) FROM recordings").fetchone()[0]
        sents = con.execute("SELECT COUNT(*) FROM sentences WHERE is_active=1").fetchone()[0]
    mins,s = divmod(int(secs),60); hrs,m = divmod(mins,60)
    await update.message.reply_text(
        f"🔧 *Admin Stats*\n\n"
